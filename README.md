...
| `sys.focus(slot)` | 前面アプリの切替要求 (キュー経由)。P4b でランチャー専用権限になる予定 |
...
- **スロット**: 0 = ランチャー予約 (P4b)、1 = dev (push → 置換 → 自然終了
  1 秒後に再実行、の従来フローそのまま)、2 = 組み込み第 2 アプリ
  (`idf.py -DMQJS_APP2=p4_bg_app.js`、空文字で無効)。
...
- **画面は切替時に破棄、復帰時に `sys.onForeground` で再構築**
  (settings 級ページの再構築 ≈ 遷移アニメ 200ms に隠れる実測に基づく)。
  `ui.screen()` のスライドインはディスパッチ終了後に開始されるので、
  ウィジェットのポップインも消えた。
- **前面切替**: ステータスバーをタップで巡回 (P4a の暫定ジェスチャ。
  P4b でランチャーへの導線になる)。JS からは `sys.focus(slot)`。
- ホストでも 2 アプリ動かせる: `./run_pc ping.js pong.js`
  (components/mqjs/tools/p4_ping.js + p4_pong.js がシグナル往復の
  スモークテスト)。
