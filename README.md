...
## MQTT 経由のタスク差し替え (実機検証済み)

ホスト側 (main/task_source.c) が JS とは独立した esp-mqtt クライアントで
制御トピックを購読し、publish された JS ソースで実行中タスクを差し替える:

```bash
python3 tools/mqtt_pub.py test.mosquitto.org <タスクトピック> examples/bench.js
```

- 受信 → 実行中スクリプトを `mqjs_runtime_stop()` で中断 → 新品の
  コンテキストで新タスクを即実行。電源を切るまで有効 (flash には書かない)。
- ブローカーとトピックは `menuconfig` → **mqjs platform** で設定
  (トピック空で機能無効)。スクリプトは 1 MQTT パケット (32KB) まで。
- 受理/拒否は `<トピック>/status` に publish される。
- **署名検証はまだ無い**。パブリックブローカーではトピック名だけが
  秘密なので、本番運用前にロードマップの Ed25519 署名検証を入れること。
...
- LittleFS パーティション + タスクのダウンロード/署名検証 (Ed25519)
  — MQTT 経由の無署名差し替えは実装済み (上記)。残りは永続化と署名
