...
- **スロット**: 0 = ランチャー (examples/launcher.js を組み込み、常駐・
  停止不可・死んでも自動再起動)、1 = dev (push → 置換 → 自然終了 1 秒後に
  再実行、の従来フローそのまま)、2-3 = ランチャー / `sys.launch` から
  起動するアプリ。`idf.py -DMQJS_APP2=p4_bg_app.js` は起動可能な組み込み
  ソースとして登録される (空文字で無効)。
...
- **アプリのインストール (P4b-lite)**: push payload の先頭行が
  `// @app <name>` なら実行せず `/littlefs/apps/<name>.js` に保存され、
  ランチャー / `sys.launch(name)` から起動できる
  (配布 topic は「保存まで」、即時置換は dev 専用 — 設計 §6)。
