...
flowchart LR
    ...
    RT --> L["ランチャー<br/>常駐 (kind: system)"]
    RT --> D["開発タスク<br/>MQTT push で即時差替"]
    RT --> A["汎用 worker ×2<br/>インストール済みアプリ"]
...
JavaScript アプリの worker 割当、起動、停止、背景イベントは esp32p4-mqjs 独自ランタイムが管理し、
...
`sys.apps()` は実行中アプリの `{name, kind, evictable}` を返します。
...
ランチャーの常駐・停止不可、開発タスクの自然終了リラン、自動起動 (`@autostart`)
は、すべてアプリごとの policy として管理されます (App Manager Phase 3)。

空き worker が無い状態で起動を要求すると、背面で最も長く使われていない
evictable なアプリが自動停止して枠を譲ります (LRU eviction、Phase 4)。
立ち退きはコンソールと「evicted: <名前>」通知で可視化され、ランチャーから
ワンタップで復帰できます。

アプリは `sys.onStop(function (reason) { ... })` で停止直前の通知を受け取れます
(reason: `"user"` / `"idle"` / `"updated"` / `"evicted"` / `"error"`)。
ここで `store.set()` に状態を保存しておき、次回起動時に復元するのが定石です。
登録してもアイドルなアプリの延命にはなりません。
...
| トピック | 用途 | 動作 |
|---|---|---|
| `TASK_TOPIC` | 開発タスク | 署名検証後、開発タスク worker を即時置換して LittleFS に保存 |
