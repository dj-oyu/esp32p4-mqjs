...
- **`// @autostart` ディレクティブ** (2026-06-11 ユーザー合意):
  manifest に 1 行足すと、起動時にそのアプリを自動 launch する。
  動機 = clip_mirror のような常駐ブリッジが再起動のたびに手動起動に
  なるのが地味に効く。注意点 (実装時に検討):
  - 「配布物は自動実行しない」(§6) との整合 — autostart は
    **インストール時ではなくブート時**の話なので原則とは両立する。
    ただし棚から新規に届いた @autostart アプリが「次の再起動で勝手に
    走り出す」ことになるので、インストール通知にその旨を出すか、
    ローカルで一度ユーザーが起動したアプリだけ autostart を有効化する
    (opt-in 記録を store/NVS に持つ) かを決める。後者が無難。
  - スロット予算: 常駐 launcher (slot 0) + dev (slot 1) + autostart
    群で 4 スロットが埋まりうる。autostart は残りスロットの範囲で
    起動順 = インストール順、溢れたら通知だけ。
  - 失敗時 (構文エラー等) はブートを止めず通知のみ。
- T3c システムパネル (ssh-terminal-design §7 T3c): stats +
  クリップボードプレビュー + lv_arc 明るさダイヤル。
