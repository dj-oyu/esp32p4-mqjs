...
Status: **Phase T1 実装完了** (2026-06-10、ビルド/PC スモーク済み。実機 SSH 接続テストはユーザーのサーバ情報待ち)
...
| **T1. 接続 + 素朴な端末 ✅** (2026-06-10) | sshc セッションタスク、ssh.* バインディング、examples/ssh_term.js (印字+改行+BS の行バッファ) | JS だけで SSH シェルに繋ぎ、コマンド出力が画面に流れる (実機テスト待ち) |
| **T2. VT100 パーサ** | JS 側でカーソル移動 (CUU/CUD/CUF/CUB)、消去 (ED/EL)、SGR カラー、スクロール領域を解釈。固定グリッド (行×桁) で上書き描画 | vi / top / ls --color がまともに見える |
