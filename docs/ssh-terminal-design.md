...
Status: **Phase T2 実装完了** (2026-06-11、VT100 パーサ+固定グリッド。PC パーサ検証済み、実機 vi/top 確認待ち)
T1 (素朴な行端末) は実機 SSH ログイン成功確認済み (2026-06-11)。
...
| **T2. VT100 パーサ ✅** (2026-06-11、実機確認待ち) | `examples/ssh_vt.js`: 80x24 セルグリッド + エスケープ状態機械 (チャンク跨ぎ)。CSI カーソル移動 (A/B/C/D/G/d/H/f)、消去 (J/K)、SGR 16 色+bold+inverse、スクロール領域 (r)、行/文字 挿入削除 (L/M/P/@)、ESC index/reverse-index/save-restore。固定セル幅でグリッド描画、行ダーティ + レート制限フラッシュ (~30fps, 4 行/tick) | vi / top / ls --color がまともに見える |
| **T3. 仕上げ** | 制御キー行 (Esc/Tab/Ctrl/矢印) をキーボードに追加、ホスト鍵 TOFU 保存、公開鍵認証、接続情報の設定 UI | 実用的に使える |
