...
| `ssh_vt.js` | 要 WiFi + sshd (Tab5 画面で操作) | ハイブリッド | SSH ターミナルエミュレータ (Phase T2/T3)。VT100 パーサ + セルグリッド + 16 色。ls --color / vi / top 対応。T3a: キーボード上のコントロールバー (Esc/Tab/one-shot Ctrl·Alt/Fn→F1-F12/矢印/Paste=clipboard)。接続フォーム入力、切断でフォームに復帰。`SELFTEST=true` で PC パーサ検証 |
